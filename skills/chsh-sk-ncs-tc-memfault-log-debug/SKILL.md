...
Router SSH: `paramiko` to `192.168.92.1` — credentials from `chsh-sk-router-control/config.json`.
AP interface: `wl1.1`.
...
whether any facts in this skill are new, corrected, or outdated (e.g. new root
causes discovered, Memfault API endpoint changes, device serial additions, or
new fix verification procedures).
