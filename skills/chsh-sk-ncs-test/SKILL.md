...
# chsh-sk-ncs-test — QA & Functional Test
...
`~/.claude/skills/chsh-sk-ncs-debug/scripts/loop_test.py` and edit the constants at
the top.
...
> Reply **yes** to continue with chsh-sk-ncs-workflow, or **no** to stop here."
...
- `chsh-sk-ncs-debug` — UART log capture, loop test scripts, multi-device comparison, barrier debugging
- `chsh-sk-ncs-workflow` — full lifecycle orchestrator; routes back here after Phase 3
- `chsh-sk-ncs-prd` — update `docs/pm-prd/PRD.md` if requirements need changing
- `chsh-sk-ncs-spec` — update `docs/dev-specs/` if design gaps are found
- `chsh-sk-ncs-project` — fix code for P0 issues
- `chsh-sk-git-release` — once tests pass, tag and publish a release
