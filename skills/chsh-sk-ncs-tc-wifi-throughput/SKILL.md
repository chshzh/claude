...
```bash
SKILL=~/.claude/skills/chsh-sk-ncs-tc-wifi-throughput
SCRIPT=$SKILL/scripts/run_throughput_test.py
...
| `PASS` | `<wifi-password>` | Wi-Fi password — see `chsh-sk-router-control/config.json` |
...
Connect to Wi-Fi first (password from `chsh-sk-router-control/config.json`):
```
wifi connect -s BE92U_2.4G -k 1 -p <wifi-password>
...
whether any facts in this skill are new, corrected, or outdated (e.g. throughput
baseline updates after sQSPI fixes, script path changes, new board configs,
or Jira field name changes).
