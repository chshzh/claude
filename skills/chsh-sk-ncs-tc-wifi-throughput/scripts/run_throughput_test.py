...
Usage: python3 -u ~/.claude/skills/chsh-sk-ncs-tc-wifi-throughput/scripts/run_throughput_test.py <serial_port> <device_label>
Prereq: have `iperf -s -u` already running in a terminal (for TX measurement).
"""
import serial, time, re, sys, subprocess, json, os
...
# Load per-lab config (contains credentials — not committed to git)
_cfg_path = os.path.join(os.path.dirname(__file__), "..", "config.json")
try:
    with open(_cfg_path) as _f:
        _cfg = json.load(_f)
except FileNotFoundError:
    print(f"ERROR: config.json not found at {_cfg_path}")
    print("Copy config.json.template → config.json and fill in your lab values.")
    sys.exit(1)

SSID_24  = _cfg.get("ssid_24", "EX75_2.4G")
SSID_5   = _cfg.get("ssid_5",  "EX75_5G")
PASS     = _cfg["wifi_password"]   # required — no default
MAC_IP   = _cfg["mac_ip"]          # required — no default
