...
See [`references/wiki-templates.md`](references/wiki-templates.md) for the full
SCHEMA.md, index.md, and log.md templates customized to the domain.
...
- **Index omission**: Every new page MUST be added to `index.md` immediately. Skipping it causes lint to flag the page as an orphan even when it has inbound links.
- **Log is append-only**: Never delete or reorder `log.md` entries. If an action was logged by mistake, add a correction entry.
- **sha256 on re-ingest**: Verify sha256 matches before overwriting. A mismatch means the source changed — bump the ingest date and note the delta.
- **`[[wikilinks]]` without targets**: Creating a wikilink to a non-existent page silently creates a broken link. Create the target page immediately or use plain text until it's ready.
- **Tag taxonomy drift**: New tags introduced during ingest must be added to `SCHEMA.md` first. Tags missing from the taxonomy fail lint.
...
whether any facts in this skill are new, corrected, or outdated (e.g. ingestion
workflow changes, new source formats, file naming conventions, or wiki directory
layout changes).
