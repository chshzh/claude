...
# chsh-ag-skill-review — Daily Skill Health Audit
...
> **Rule authoring**: This skill uses the rules defined in `chsh-ag-skill-create`
...
For each personal skill, verify against `chsh-ag-skill-create` rules:
...
Patterns to detect:
- A skill mentions "debug" or "UART" without referencing `chsh-dev-ncs-debug`
- A skill mentions "git commit" or "push" without referencing `chsh-dev-git-commit`
- A skill mentions "release" or "tag" without referencing `chsh-dev-git-release`
- A skill mentions "PRD" or "requirements" without referencing `chsh-pm-ncs-prd`
- A skill mentions "specs" without referencing `chsh-dev-ncs-spec`
- A skill talks about "QA" or "test report" without referencing `chsh-qa-ncs-test`
...
Produce a Markdown report at `~/.claude/skills/chsh-ag-skill-review/report-YYYY-MM-DD.md`:
...
| Commit reviewed skill fixes | `chsh-dev-git-commit` |
| Push to claude repo | `chsh-dev-git-commit` (Step 5 — push) |
